...
local function open_channel(t, key)
    ...
    --可能节点还没启动完成 连接不上重试10秒

    local i = 0
    local ok,connected = pcall(c.connect,c,true)
    print("open_channel",i,ok,connected)

    while i < 10 and not ok or not connected do
        print("open_channel",i)
        i = i + 1
        skynet.sleep(100)
        ok,connected = pcall(c.connect,c,true)
    end
    assert(i < 10)
    t[key] = c
    return c
